...
SCANNER_BR_ACOES = [
    ...
    "GGBR4.SA", "LREN3.SA", "EQTL3.SA", "RADL3.SA", "VBBR3.SA"import streamlit as st
import pandas as pd
import google.generativeai as genai
import requests
...
# INSIRA SEU TOKEN DA BRAPI AQUI (Pegue grátis em brapi.dev)
BRAPI_TOKEN = "xKJf3tRSyn6uf6WdurNbt" 
...
def analisar_saude_ativos(lista_tickers):
    ...
    # Formata a lista unindo por vírgulas para fazer apenas UMA requisição rápida
    ...
    try:
        ...
        st.error(f"Erro na conexão com o provedor de dados: {e}")
        return pd.DataFrame()
...
if menu == "🎯 Carteira Recomendada":
    ...
    if st.button("Escanear Mercado Agora"):
        with st.spinner("Conectando com a API de alta velocidade..."):
